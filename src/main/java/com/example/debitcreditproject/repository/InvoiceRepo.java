...
public interface InvoiceRepo extends JpaRepository<InvoiceEntityDetails, String> {
	@Modifying
	@Query(value = "update invoice_entity_details set status = :status where invoice_number = :invoiceNumber", nativeQuery = true)
	void updateStatus(@Param("status") String status, @Param("invoiceNumber") String invoiceNumber);

	@Query(value = "select status from invoice_entity_details where invoice_number=?1", nativeQuery = true)
	String findStatus(String invoiceNumber);

	@Modifying
	@Query(value = "update invoice_entity_details set booking_status = :bookingStatus where invoice_number = :invoiceNumber", nativeQuery = true)
	void updateBookingStatus(@Param("bookingStatus") String bookingStatus,
			@Param("invoiceNumber") String invoiceNumber);

	@Query(value = "select booking_status from invoice_entity_details where invoice_number=?1", nativeQuery = true)
